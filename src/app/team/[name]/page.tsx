...
// Per-person detail — the single drill-down for a team member. Reached
// from the /team grid card, the global Sidebar search, and from /status
// (which redirects /status/[name] here for back-compat). Detail (sessions,
// tools, tokens, stuck quotes, work_summary) is fine because it is pull,
// not push. Still no persona / MBTI / coaching metrics — those belong in
// a 1:1 or the person's own self-view.
...
function fmtDateTime(iso: string | null | undefined): string {
  ...
  // Pin to Beijing (UTC+8) — team operates there; raw local/UTC causes
  // "is this stale?" confusion.
...
export default function AgentDetailPage() {
  ...
  useEffect(() => {
    document.title = `${name} · Team · Rocket Team`;
  }, [name]);
  ...
  return (
    <div className="px-12 py-10 max-w-[1040px] mx-auto">
      ...
      <div className="eyebrow mb-6">
        Rocket Team / <Link href="/team" className="hover:text-ink-muted transition-colors">Team</Link> / {name}
      </div>
        ...
        <>
          ...
          {d.activityFlag === 'never' ? (
            ...
            <section className="rounded-xl border border-dashed border-rule bg-paper-card px-8 py-14 text-center">
              <p className="font-serif text-[14px] text-ink mb-1.5">No Claude Code session data yet</p>
  ...
  return (
    <div className={`flex items-start gap-3 px-4 py-3 ${done ? 'opacity-65' : ''}`}>
      ...
      <div className="min-w-0 flex-1">
        <div className={`text-[13px] leading-snug ${done ? 'text-ink-soft' : 'text-ink font-medium'}`}>{it.title}</div>
...
// §S6 — "本周参与项目" chips. Fetched async because the project list is
// derived from cached work-summary attribution, not from the agent detail
// payload. Renders nothing until the fetch resolves (no skeleton — chips are
// secondary signal, leader's eye lands on activity badge first).
function ProjectChips({ agentName }: { agentName: string }) {
  const [items, setItems] = useState<Array<{ id: string; name: string }>>([]);
  useEffect(() => {
    let cancelled = false;
    void (async () => {
      try {
        const res = await fetch(`/api/agents/${encodeURIComponent(agentName)}/projects`, {
          cache: 'no-store'
        });
        if (!res.ok) return;
        const data = (await res.json()) as { projects?: Array<{ id: string; name: string }> };
        if (!cancelled && Array.isArray(data.projects)) setItems(data.projects);
      } catch {
        /* chips are decorative; swallow */
      }
    })();
    return () => {
      cancelled = true;
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-1.5">
      <span className="eyebrow text-ink-quiet">本周参与项目</span>
      {items.map((p) => (
        <Link
          key={p.id}
          href={`/status/project/${encodeURIComponent(p.id)}`}
          className="text-[11px] px-2 py-0.5 rounded-full bg-paper-subtle text-ink-soft hover:bg-paper-card border border-rule transition-colors"
        >
          {p.name}
        </Link>
      ))}
